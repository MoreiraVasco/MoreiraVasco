...
class Model(object):
    ...
    def update(self, dt):
        ...
        if dtheta == 0:
            ...
            dy = linear_speed * dt * math.sin(dtheta)/ dtheta

            dp = linear_speed * dt * (math.sin(dtheta)-1)/ dtheta

       # Updating the robot position
        self.x = self.x + dp*(math.cos(self.theta) - dy*math.sin(self.theta))
        self.y = self.y + dp*(math.cos(self.theta) + dy*math.sin(self.theta))
